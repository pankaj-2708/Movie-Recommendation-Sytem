...
df1 = pd.read_csv("./data/processed/Frontend.csv")
...
def allmovies():
    res = requests.get(f"{api_url}/allmovies")
    resT = res.json()
    return resT


def RecommendStory(movie):
    res = requests.get(f"{api_url}/story?movie={movie}")
    resT = res.json()
    return resT["similar_movies"], resT["posters"], resT["date"]


def Recommendcast(movie):
    res = requests.get(f"{api_url}/cast?movie={movie}")
    resT = res.json()
    return resT["similar_movies"], resT["posters"], resT["date"]


def Recommendscale(movie):
    res = requests.get(f"{api_url}/scale?movie={movie}")
    resT = res.json()
    return resT["similar_movies"], resT["posters"], resT["date"]
